...
const Editor = () => {
  ...
  const handleChange = (ev: React.ChangeEvent<HTMLInputElement>, index: string) => {
    
    const temp = actualClass;
    
    if(temp)
      temp.set(index, ev.target.value);
    ...
    <>
      ...
      <div ref={editor} className={editorClasses}>
        <form className='flex flex-wrap justify-between gap-2 bg-slate-400 w-full h-max p-4 pb-20' action="">
          { actualClass && Object.entries(actualClass.headers).map((header) => <input onChange={event => handleChange(event, header[0])} className='w-9/20 2xl:w-6/20 3xl:w-4/20 4xl:w-3/20 rounded-lg p-2 placeholder:capitalize' type="text" value={item ? item[header[0]] : ''} placeholder={header[1]} name={header[1]} id={header[0]} key={header[1]}/>)}
